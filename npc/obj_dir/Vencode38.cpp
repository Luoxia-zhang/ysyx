...
std::unique_ptr<VerilatedTraceConfig> Vencode38::traceConfig() const {
...
void Vencode38___024root__trace_init_top(Vencode38___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vencode38___024root__trace_init_top(vlSelf, tracep);
...
VL_ATTR_COLD void Vencode38::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vencode38::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vencode38___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
